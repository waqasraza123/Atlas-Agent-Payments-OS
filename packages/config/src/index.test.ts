...
describe("atlas config", () => {
  ...
  it("uses fallback runtime values when env is not set", async () => {
    vi.unstubAllEnvs();
    const { apiRuntime, paymentRuntime, programmableSettlementRuntime, storageRuntime, workerRuntime } = await import("./index");
  ...
  it("reads runtime values from the environment", async () => {
    ...
    const { apiRuntime, paymentRuntime, programmableSettlementRuntime, storageRuntime, workerRuntime } =
      await import("./index");
