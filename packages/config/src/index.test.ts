...
describe("atlas config", () => {
  ...
  it("uses fallback runtime values when env is not set", async () => {
    ...
    expect(observabilityRuntime.automationRemediationFollowUpMinutes).toBe(60);
  ...
  it("reads runtime values from the environment", async () => {
    ...
    vi.stubEnv("OBSERVABILITY_TELEMETRY_REMEDIATION_FOLLOW_UP_MINUTES", "90");
    ...
    expect(observabilityRuntime.automationRemediationFollowUpMinutes).toBe(90);
  ...
  it("rejects invalid telemetry remediation follow-up windows", async () => {
    const { validateAtlasRuntimeConfiguration } = await import("./index");
    ...
    const result = validateAtlasRuntimeConfiguration("worker", {
      ...
      OBSERVABILITY_AUTOMATION_STARTUP_DELAY_SECONDS: "30",
      OBSERVABILITY_AUTOMATION_TELEMETRY_POLICY: "recover",
      OBSERVABILITY_AUTOMATION_TELEMETRY_ESCALATION_THRESHOLD: "2",
      OBSERVABILITY_TELEMETRY_REMEDIATION_FOLLOW_UP_MINUTES: "0",
      OBSERVABILITY_AUTOMATION_ACTOR_USER_EMAIL: "operator-admin@atlas.local",
      OBSERVABILITY_AUTOMATION_REASON: "Run scheduled observability automation for the current release slot."
    });
    ...
    expect(result.issues).toContainEqual(
      expect.objectContaining({
        variable: "OBSERVABILITY_TELEMETRY_REMEDIATION_FOLLOW_UP_MINUTES"
      })
    );
  });
