...
- Support-session tenant-read audit now exists across buyer and seller workspace overview, surface, and detail reads through web-server audit events that preserve principal and target-tenant context
...
- Direct buyer and seller web route pages now use actor-aware shared workflow wrappers as well, and seller overview analytics reads now inherit support-session tenant audit from the shared seller workflow
...
- Broader continuous telemetry ownership beyond the current scheduler-enforced policy, repeated-breach escalation, same-cycle recovery escalation, failure-path escalation artifacts, guided operator remediation, explicit remediation ownership and closure, remediation-queue follow-through, acknowledgement follow-up SLA enforcement, incident-closure reconciliation, overdue-handoff remediation escalation, durable escalation ownership history, explicit owner assignment and transfer, owner follow-through tracking, reassignment accountability outcomes, and retained repo artifacts
