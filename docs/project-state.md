...
- Timer-driven observability automation and retention policy baseline now exist through worker-scheduled automation runs, durable automation-run reports, explicit per-artifact retention windows, automated retention sweeps, and operator-visible automation schedule and history
- Broader web and external-integration trace propagation plus repo-owned external paging now exist through shared web trace generation, trace-aware operational adapters, operator-visible dispatch delivery metadata, and owned PagerDuty/Opsgenie paging adapters
...
- Support-session tenant-read audit now exists across buyer and seller workspace overview, surface, and detail reads through both web-server route audit events and shared database workflow audit events that preserve principal and target-tenant context
- Buyer and seller workspace detail reads now enforce tenant-scoped database filters before record loading, preventing cross-tenant fetches from being materialized ahead of in-memory checks
- Shared payment and receipt detail reads now enforce actor-scoped tenant filters at the database workflow layer before records are loaded by id or request id
- Support-session payment and receipt reads now emit tenant-access audit events directly from the shared payments workflow so API and web consumers inherit the same support inspection evidence
- Buyer and seller API list reads now use actor-aware shared workflow wrappers so support-session inspection of agents, policies, requests, approvals, services, team members, profile, and seller requests is audited consistently at the database layer
- Direct buyer and seller web route pages now use actor-aware shared workflow wrappers as well, seller overview analytics reads now inherit support-session tenant audit from the shared seller workflow, and buyer plus seller workspace overview and surface loaders now also resolve through shared actor-aware workspace read wrappers instead of web-layer Prisma tenant reads
- Seller API analytics and service-detail reads now also use actor-aware shared workflow wrappers so support-session inspection stays audited across those remaining API detail entrypoints
- Buyer and seller request detail plus seller service, payment, and receipt detail routes now enter through shared actor-aware workflow gates before the richer web detail graph loads, so support-session inspection inherits database-layer tenant audit across top-level detail reads as well
- Buyer approval detail now also enters through a shared actor-aware workflow gate and a protected API detail route, so approval inspection inherits database-layer tenant audit and organization scope before the richer web detail graph loads
- Buyer activity detail now also enters through a shared actor-aware workflow gate and a protected audit-event detail route, while operator audit detail now enters through a shared operator reader before the richer web detail graph loads
...
- Recover-mode observability automation now syncs incidents and optional dispatch from the post-recovery alert set, so incomplete recovery can trigger same-cycle incident and paging posture instead of waiting for the next run
- Recover-mode observability automation failures now also synthesize same-cycle snapshot, incident-trigger, and optional dispatch posture from the failure state, so hard failures can still raise immediate operator escalation even before post-recovery state is computed
- Operator observability now includes a guided telemetry-remediation plan plus a one-click recommended response, so operators can execute the current owned recovery path directly from remediation posture instead of inferring the next step from raw alerts and history
- Operator observability now also persists telemetry-remediation acknowledgement and closure actions with retention-aware remediation artifacts, derived owner state, and operator-visible history so ownership and final closure are durable and auditable instead of inferred from automation runs alone
- Telemetry remediation now also stays synced into the operator notification queue with observability-linked deep links and audit-backed acknowledgement or closure actions, so remediation handoff is visible beyond the alerts page itself
- Telemetry remediation now also carries an explicit acknowledgement follow-up window with overdue operator alerts, overdue-notification resurfacing, and operator-visible timing facts so acknowledged issues cannot linger silently after handoff
- Telemetry remediation resolution now also reconciles current observability incident-trigger posture and records how many linked incidents were resolved or remain active, so closure cannot drift away from stale incident state
- Materially overdue acknowledged telemetry remediation now also escalates the guided remediation plan itself into dispatch-capable recovery, so aged handoffs become an active escalation workflow instead of only a side-channel alert
- Overdue telemetry remediation escalation now also persists explicit escalation events plus operator re-acknowledgement history, so aged handoffs have durable named ownership instead of relying only on transient alerts
- Telemetry remediation escalation now also supports explicit assignment and transfer to validated operator owners, with durable handoff records and owner-state facts on `/operator/alerts`
- Telemetry remediation now also tracks owner follow-through after assignment or transfer, exposing whether the named operator has acted since handoff across alerts, notification metadata, and the operator observability surface
- Telemetry remediation reassignment now also persists whether the previously assigned owner met or missed the follow-through expectation before handoff, and Atlas promotes unmet accountability into notification metadata, operator history, and observability alerts
- Manual telemetry remediation recovery, re-acknowledgement, and closure now enforce named-owner discipline, so another operator must transfer ownership before acting on an already-owned remediation posture
- Operator observability now also exposes protected API write routes for guided telemetry recovery and remediation ownership actions, so external operator clients use the same owned recovery and handoff workflows as the web control plane
...
- Next active implementation track: remaining tenant-boundary hardening plus deeper telemetry ownership for broader real-world rollout
...
- Continuous telemetry ownership history and timeline-derived posture are now in place in repo scope
- Broader web and external-integration trace propagation plus repo-owned external paging are now in place in repo scope
- The current active execution slice is broader production observability maturity and any remaining tenant-boundary hardening for broader real-world rollout
...
- Release verification now also includes `pnpm verify:tenant-boundaries` so guarded workspace loaders cannot regress back to direct Prisma tenant reads
- Runtime telemetry ownership now persists append-only API, worker, and automation ownership samples with retention-aware history, timeline-derived breach windows, breach-age-aware operator alerts, and `pnpm verify:ops` ownership freshness checks
...
- Third-party APM tooling, external paging lifecycle synchronization, and broader cross-system observability ownership beyond the current repo-owned ownership timeline and operator workflows
...
- `pnpm verify:release` now validates env templates, tenant-boundary guards, rollback-readiness, test/build gates, and standalone web production build
