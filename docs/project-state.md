...
- Rollout execution visibility now exists through an operator rollout surface, restore-drill execution reports, secret-rotation execution reports, promotion execution reports, upstream identity execution reports, and a persisted operational integration registry with active and verified execution gating
...
- Next active implementation track: vendor-native rollout execution ownership, stored non-local proof artifacts, secret-manager-native rotation execution, and environment-specific deployment runner integration
...
- The current active execution slice is vendor-native rollout execution ownership, stored non-local proof artifacts, secret-manager-native rotation execution, and environment-specific deployment runner integration
...
- Direct external identity-provider lifecycle operations beyond the current assignment, session, and provider-link governance baseline
...
- The current rollout-hardening baseline now adds signed session handling, direct external OIDC exchange into persisted Atlas sessions, assignment-backed external identity provisioning and deprovisioning, reviewable and revocable support grants, support-mode read-only enforcement across API and workflow layers, tenant-targeted support access, explicit activation, recertification support, campaign-driven access review, identity-session revocation, identity-link lifecycle governance, actor-scoped reporting and export enforcement, tenant-read audit events across analytics and support-governance paths, upstream identity execution reports, provider-aware adapter scripts for upstream identity, restore drills, secret rotation, and deployment promotion, a persisted operational integration registry with active and verified execution gating, shared promotion bundle generation, promotion-manifest generation, artifact-bound release metadata, backup integrity manifests, proof-bearing restore-drill reports with execution metadata, secret-rotation execution reports, request correlation, runtime health surfaces, metrics, operator alert posture, incident runbooks, security headers, and CI release verification, but vendor-native live upstream IdP ownership, stored non-local restore ownership, and secret-manager-native rotation ownership are still not in place
- Database backup and restore scripts now exist, restore drills now emit proof-bearing reports with execution metadata and provider-aware adapter results, and command-mode execution now requires an owned verified rollout target, but scheduled backups, stored non-local restore proof artifacts, and cloud deployment automation are still not in place
