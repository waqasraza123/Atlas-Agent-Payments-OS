...
import {
  AtlasAuthSessionWorkflowError,
  exchangeExternalIdentityForSession,
  exchangeIdentityAssertionForSession
} from "@atlas/database";
...
function resolveWorkflowStatus(code: AtlasAuthSessionWorkflowError["code"]) {
  if (code === "bad_request") {
    return 400;
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (error instanceof AtlasAuthSessionWorkflowError) {
      return new NextResponse(error.message, {
        status: resolveWorkflowStatus(error.code)
      });
    }
